...
void FIWeightIndexFinderPass(std::map<llvm::Instruction *, std::__cxx11::list<int> *> *fi_inst_regs_map){
    ...
    for (std::map<Instruction*, std::list< int >* >::const_iterator
       ...
       inst_reg_it != fi_inst_regs_map->end(); ++inst_reg_it) {
        Instruction *fi_inst = inst_reg_it->first;
        Instruction *g_inst = nullptr; // This will hold the getelementptr instruction

        // Start from the fmul instruction and iterate backwards through the basic block
        int cnt = 0;
        for (auto it = fi_inst->getIterator().getReverse(), et = fi_inst->getParent()->rend(); it != et; ++it) {
            // Check if the instruction is a 'getelementptr'
            if (isa<GetElementPtrInst>(&*it)) {
                g_inst = &*it; // Found the getelementptr instruction
                // if(cnt == 1)
                  break; // Break out of the loop once we've found what we're looking for
                cnt ++;
            }
        }
        

        // Assume g_inst is a pointer to the getelementptr instruction (Instruction* g_inst)
        LLVMContext &context = g_inst->getContext();
        Module *M = g_inst->getModule();

        // Get the last argument of the getelementptr instruction
        Value *gepLastArg = g_inst->getOperand(g_inst->getNumOperands() - 1);

        // Declare the function type for indMMM
        FunctionType *indMMMType = FunctionType::get(Type::getVoidTy(context), {Type::getInt64Ty(context)}, false);

        // Get or insert the declaration for indMMM
        FunctionCallee indMMMFunc = M->getOrInsertFunction("DLAWeightIndex", indMMMType);

        // Ensure the function declaration is correct (this step is optional but recommended)
        if (Function *indMMM = dyn_cast<Function>(indMMMFunc.getCallee())) {
            indMMM->setCallingConv(CallingConv::C); // Set the calling convention if needed
        }

        // Create the call instruction to indMMM
        CallInst *callIndMMM = CallInst::Create(indMMMFunc, {gepLastArg}, "", g_inst->getNextNode());
      }
    }
...
bool FaultInjectionPass::runOnModule(Module &M) {
  ...
  FIWeightIndexFinderPass(fi_inst_regs_map);
  finalize(M);
  return true;
}
