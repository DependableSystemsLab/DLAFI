/*
 * input.c - Sample program for bert
 *
 *
 *
 */
...
#define NUM_INPUTS 2 // When using the same input.c file for a different model, specify the number of inputs here depending on the model
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < NUM_INPUTS; i++) {
	    ...
	    // When using the same input.c file for a different model, Change "int64_t" to the type of input data that the model expects
	    int64_t* input_data {reinterpret_cast<int64_t *>(const_cast<char*>(input.raw_data().data()))};
	    int64_t* heap_input_data;
	    ...
	    heap_input_data = (int64_t*)malloc(sizeof(int64_t) * totalSize);
	    memcpy(heap_input_data, input_data, totalSize * sizeof(int64_t));


	    //When using the same input.c file for a different model, Change ONNX_TYPE depending on the input type
	    OMTensor *x1 = omTensorCreate(heap_input_data, in_shape, d.size(), ONNX_TYPE_INT64);
	    img_list[i] = x1;
