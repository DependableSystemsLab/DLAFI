...
static struct {
  ...
  int fi_Mac_x;
  int fi_Mac_y;
} config = {"bitflip", false, -1, -1, -1, 15, 1, -1, -1, {-1}, -1, "", -1, -1};
...
void _parseLLFIConfigFile() {
  ...
  while (fgets(line, CONFIG_LINE_LENGTH, ficonfigFile) != NULL) {
    ...
    //======== Add MAC unit position========
    else if (strcmp(option, "fi_MAC_x") == 0) {
      config.fi_Mac_x = atoi(value);
      assert(config.fi_Mac_x >= 0 && "invalid fi_bit in config file");
    }
    else if (strcmp(option, "fi_MAC_y") == 0) {
      config.fi_Mac_y = atoi(value);
      assert(config.fi_Mac_y >= 0 && "invalid fi_bit in config file");
    //======== Add number of corrupted bits QINING @MAR 13th========
...
void initInjections() {
  ...
  read_indices(config.fi_Mac_x,config.fi_Mac_y);
...
bool preFunc(long llfi_index, unsigned opcode, unsigned my_reg_index,
  ...
  // //lets say only considering MAC fAULTS 
...
void injectFunc(long llfi_index, unsigned size, char *buf,
    ...
    fi_bitpos = rand();
    if(config.fi_bit >= 0)
      fi_bitpos = config.fi_bit;
    fi_bytepos = fi_bitpos / 8;
    fi_bitpos %= size;
    oldbuf = buf[fi_bytepos];
    union inputBuffer oldVal = {.f = *((float*)buf)};
    
    int8_t val = buf[fi_bytepos];
    int shift = fi_bitpos % 8;
    if (strcmp(config.fi_type, "stuck_at_1") == 0) {
      val |= (0x1 << shift);
    }
      ...
      fprintf(stderr,"fault type not recognized: %s\n",config.fi_type);
      // fprintf(stderr, "YOOOOOOOOOOOO\n");
    ...
    union inputBuffer newVal = {.f = *((float*)buf)};
...
void postInjections() {
  ...
  char numfaultsfilename[80];
  strncpy(numfaultsfilename, "llfi/numfaults.txt", 80);
  numfaultsfileFile = fopen(numfaultsfilename, "a");
  fprintf(stderr,"%d faults were injected\n",numfaults);
  fprintf(numfaultsfileFile,"%d faults were injected\n",numfaults);
  fclose(injectedfaultsFile);
