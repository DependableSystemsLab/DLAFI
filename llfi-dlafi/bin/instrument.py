...
def checkInputYaml():
  ...
  try:
    ...
    print("Error: input.yaml is not formatted in proper YAML (reminder: use spaces, not tabs1)")
    os.rmdir(options["dir"])
    exit(1)
...
def readCompileOption():
  ...
  if (dOpt != None) and ("deviceType" in dOpt):
    if (dOpt["deviceType"] == "SA") or (dOpt["deviceType"] == "Hybrid"):
        print("Non-CPU device type detected. Going to accelerator Mode.")
        isDeviceCPU = False
  print("||||||||| device type is :::::",dOpt["deviceType"])
  if (dOpt["deviceType"] == "SA") or (dOpt["deviceType"] == "Hybrid"):
    fiSAconfig_File.write("SystolicArrayDataflow="+str(dOpt["SystolicArrayDataflow"])+'\n')
    fiSAconfig_File.write("SystolicArrayDimension="+str(dOpt["SystolicArrayDimension"])+'\n')
    if("Sampler" not in dOpt):
      dOpt["Sampler"] = 1
    fiSAconfig_File.write("SystolicArraySample="+str(dOpt["Sampler"])+'\n')
    counter = 0
    if os.path.exists('./FIlocations'):
      shutil.rmtree('./FIlocations')
    os.makedirs('./FIlocations')
    valid_range = int(int(dOpt["SystolicArrayDimension"]) / int(dOpt["Sampler"]))
...
def compileProg():
  global proffile, fifile, compileOptions, defaultlinklibs, isDeviceCPU, dOpt
  ...
  if not options["IRonly"]:
    ...
    if retcode == 0:
      ...
      execlist.extend(liblist)
      execlist.extend(["-lllfi-rt"])
      execlist.extend(["-lbsd"])
      execlist.extend(["-pthread"])
      execlist.extend(["-static"])
      print("\nFI1 execlist:",execlist)
      execlist[0] = llvmgxx
      print("\nFI2 execlist:",execlist)
      retcode = execCompilation(execlist)
