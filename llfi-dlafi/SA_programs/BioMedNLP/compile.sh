...
if [ -f "$FILE" ]; then
    ...
    python3 -m transformers.onnx --model=microsoft/BiomedNLP-PubMedBERT-base-uncased-abstract-fulltext --feature=masked-lm --opset 14 .
    deactivate
fi

printf "\n[Compile Script]: Convert TF model to LLVM IR\n"
onnx-mlir --EmitLLVMIR  --instrument-onnx-ops="ALL" --InstrumentBeforeAndAfterOp model.onnx --lltfi-fi
