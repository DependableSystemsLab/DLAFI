...
if [ -f "$FILE" ]; then
    ...
    wget https://media.githubusercontent.com/media/onnx/models/refs/heads/main/validated/text/machine_comprehension/roberta/model/roberta-sequence-classification-9.onnx
fi

printf "\n[Compile Script]: Convert TF model to LLVM IR\n"
onnx-mlir --EmitLLVMIR  --instrument-onnx-ops="ALL" --InstrumentBeforeAndAfterOp roberta-sequence-classification-9.onnx --lltfi-fi
mlir-translate -mlir-to-llvmir roberta-sequence-classification-9.onnx.mlir > model.mlir.ll
