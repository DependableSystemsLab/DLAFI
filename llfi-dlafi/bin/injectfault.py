...
def checkInputYaml():
  ...
  try:
    ...
    f.close()
    usage("input.yaml is not formatted in proper YAML (reminder: use spaces, not tabs3)")
    exit(1)
  finally:
    f.close()
  ...
  if not os.path.isfile(fi_exe):
    ...
    for ii, run in enumerate(rOpt):
      ...
      with open('./llfi/numfaults.txt', 'w') as file:
        pass
      with open('./llfi/sdc.txt', 'w') as file:
        pass
      with open('llfi.config.runtime.txt', 'w') as file:
        pass
      for index in range(0, run_number):
        ...
        ficonfig_File = open("llfi.config.runtime.txt", 'a')
        ...
        if ('fi_max_multiple' in locals()):
          ...
          for index_multiple in range(1, int(selected_num_of_injection)):
            ...
            if fi_next_cycle == int(totalcycles):
              break
        ##==================================================================
        ficonfig_File.write("fi_MAC_x="+str(Mac_x)+'\n')
        ficonfig_File.write("fi_MAC_y="+str(Mac_y)+'\n')
        ficonfig_File.close()
        ...
        if(Mac_y >= SADim/int(dOpt["Sampler"])):
          ...
          break
          # fi_bit += 1
        # if(fi_bit ==32 and fi_type == "stuck_at_1"):
        #   fi_bit = 0
        #   fi_type = "stuck_at_0"
        # if(fi_bit ==32 and fi_type == "stuck_at_1"):
        #   break
