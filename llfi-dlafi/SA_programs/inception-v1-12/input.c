/*
 * input.c - Sample program for googlenet
 *
 *
 *
 */
...
#define NUM_INPUTS 1000 // When using the same input.c file for a different model, specify the number of inputs here depending on the model
...
int main(int argc, char *argv[]) {
	...
	int total = 0, sdc = 0, num_inputs = 1;
	int predicted_labels[NUM_INPUTS],true_labels[NUM_INPUTS];
	bool isProfiling=false;
	char line[1024];
	char* goldenfilename = "./llfi/baseline/goldenfile.txt";

    //Input pointers needed for the model
    char inp[NUM_INPUTS][50];
    char* savefilename = "layeroutput.txt";
    char* output_seq = NULL;
    
    unsigned int numArguments = 4; 
    if (argc >= numArguments) {
		
		if (strncmp(argv[numArguments - 1], "profiling", 9) == 0) {
            isProfiling = true;
            fprintf(stderr,"Profiling Stage\n");
        }
		
		num_inputs = atol(argv[numArguments - 3]);
		fprintf(stderr,"num_inputs=%d\n",num_inputs);
    	for (int i = 0; i < num_inputs; i++){
    		// inp[i] = ("input_" + str(i) +".pb").c_str();
			sprintf(inp[i], "input_%d.pb", i);
			fprintf(stderr,"%d= %s\n", i , inp[i]);
    	}
	output_seq = argv[numArguments-2];
    } else {
        fprintf(stderr,"Not expecting this amount of arguments.\n");
    }
    ...
    for (int i = 0; i < num_inputs; i++) {
		...
		printf("Final prediction for %i is: ", i);
		int imax = 0;
		for (int j = 0; j < 1000; j++){
			printf("%f ", outputPtr[j]);
			if(outputPtr[j] > outputPtr[imax])
				imax = j;
		}
		printf("\n");
		predicted_labels[total] = imax;
		for (void* ptr : heapAllocs) {
		free(ptr);
		}
		total += 1;
		if(total > num_inputs){
			fprintf(stderr,"WHATTTTTTTTTTTTTTTTTTTTTTTTTtt\n");
			break;
		}
    }
	fprintf(stderr,"labels: ");
	for(int i = 0 ; i < num_inputs; i++){
		fprintf(stderr,"%d ",predicted_labels[i]);
	}
	fprintf(stderr,"\n");
	if(isProfiling){
        FILE *goldenfile = fopen(goldenfilename, "w");
        for(int i = 0 ; i < num_inputs; i++){
            fprintf(goldenfile,"%d,",predicted_labels[i]);
        }
        fclose(goldenfile);
    }
	else{
        FILE *goldenfile = fopen(goldenfilename, "r");
		FILE *sdc_file = fopen("./llfi/sdc.txt", "a");
        char *token;
        if (fgets(line, sizeof(line), goldenfile)) {
        // Replace the trailing newline character with a null terminator, if it exists
            line[strcspn(line, "\n")] = 0;
            ...
            for(int i = 0 ; i < num_inputs; i++){
                ...
                token = strtok(NULL, ",");
                // fprintf(stderr,"%d ? %d,",true_labels[i],predicted_labels[i]);
                if(true_labels[i] != predicted_labels[i]){
                    sdc++;
                }
            }
            fprintf(stderr,"TOTAL SDC %d/%d\n",sdc,total);
			fprintf(sdc_file,"TOTAL SDC %d/%d\n",sdc,total);
        }
		fclose(goldenfile);
		fclose(sdc_file);
    }
