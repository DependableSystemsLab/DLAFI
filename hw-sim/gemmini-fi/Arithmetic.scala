...
abstract class ArithmeticOps[T <: Data](self: T) {
  ...
  def isNonZero(x: T): Bool // Returns true if x is non-zero, false otherwise
}
...
object Arithmetic {
  implicit object UIntArithmetic extends Arithmetic[UInt] {
    override implicit def cast(self: UInt) = new ArithmeticOps(self) {
      ...
      override def injectFault(
          tile_row: UInt, tile_col: UInt,
          pe_row: UInt, pe_col: UInt,
          do_fi: UInt,
          fi_tile_row: UInt, fi_tile_col: UInt,
          fi_pe_row: UInt, fi_pe_col: UInt,
          fi_model: UInt, fi_data: UInt
      ): UInt = {
        val fi_bit = fi_data(4, 0)

        val match_pe = (tile_row === fi_tile_row) &&
                      (tile_col === fi_tile_col) &&
                      (pe_row === fi_pe_row) &&
                      (pe_col === fi_pe_col) &&
                      (do_fi === 1.U)

        // Convert UInt to 32-bit recoded float
        val intToFloat = Module(new INToRecFN(self.getWidth, 8, 24)) // 8-bit exp, 23-bit sig + 1
        intToFloat.io.in := self
        intToFloat.io.signedIn := false.B
        intToFloat.io.roundingMode := consts.round_near_even
        intToFloat.io.detectTininess := consts.tininess_afterRounding

        val floatBits = Wire(UInt(32.W))
        floatBits := fNFromRecFN(8, 24, intToFloat.io.out)

        // Inject fault in float bits
        val faultyBits = Wire(UInt(32.W))
        faultyBits := MuxLookup(fi_model, floatBits, Seq(
          1.U -> 0.U,
          2.U -> (floatBits & ~(1.U << fi_bit)),   // stuck-at-0
          3.U -> (floatBits |  (1.U << fi_bit)),   // stuck-at-1
          4.U -> (floatBits ^  (1.U << fi_bit))    // bit-flip
        ))

        // Convert back to UInt
        val floatToInt = Module(new RecFNToIN(8, 24, self.getWidth))
        floatToInt.io.in := recFNFromFN(8, 24, faultyBits)
        floatToInt.io.roundingMode := consts.round_near_even
        floatToInt.io.signedOut := false.B

        val result = WireDefault(self)
        when(match_pe) {
          result := floatToInt.io.out
        }

        return result
      }

      override def isNonZero(x: UInt): Bool = x =/= 0.U
    }
  }
  ...
  implicit object SIntArithmetic extends Arithmetic[SInt] {
    override implicit def cast(self: SInt) = new ArithmeticOps(self) {
      ...
      override def injectFault(
        ...
        // Convert self (SInt) to float (recoded)
        val intToFloat = Module(new INToRecFN(self.getWidth, 8, 24)) // 8-bit exp, 24-bit sig = FP32
        intToFloat.io.in := self.asUInt
        intToFloat.io.signedIn := true.B
        intToFloat.io.roundingMode := consts.round_near_even
        intToFloat.io.detectTininess := consts.tininess_afterRounding

        val floatBits = Wire(UInt(32.W)) // standard float bits
        floatBits := fNFromRecFN(8, 24, intToFloat.io.out)

        // Fault injection on float bits
        val faultyBits = Wire(UInt(32.W))
        faultyBits := MuxLookup(fi_model, floatBits, Seq(
          1.U -> 0.U,                                // zero out
          2.U -> (floatBits & ~(1.U << fi_bit)),    // stuck-at-0
        ...
        val result = WireDefault(self)
        when(match_pe) {
          result := floatToInt.io.out.asSInt()
        }

        return result
      }
      override def isNonZero(x: SInt): Bool = x =/= 0.S
    }
  }
  ...
  implicit object FloatArithmetic extends Arithmetic[Float] {
    ...
    override implicit def cast(self: Float): ArithmeticOps[Float] = new ArithmeticOps(self) {
      ...
      override def injectFault(
        ...
        val original_bits = self.bits
        val faulty_bits = Wire(UInt(original_bits.getWidth.W))

        faulty_bits := MuxLookup(fi_model, original_bits, Seq(
          1.U -> 0.U,                                // Zero out
          2.U -> (original_bits & ~(1.U << fi_bit)), // Stuck-at-0
          3.U -> (original_bits |  (1.U << fi_bit)), // Stuck-at-1
          4.U -> (original_bits ^  (1.U << fi_bit))  // Bit-flip
        ))

        val out = Wire(Float(self.expWidth, self.sigWidth))
        out.bits := Mux(match_pe, faulty_bits, original_bits)
        return out
      }

      override def isNonZero(x: Float): Bool = {
        val raw = rawFloatFromFN(self.expWidth, self.sigWidth, x.bits)
        !raw.isZero && !raw.sign
      }
    }
  }
